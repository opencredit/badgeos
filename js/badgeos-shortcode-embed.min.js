...
"")}function badgeos_insert_shortcode(){var shortcode,requested;shortcode=$("#select_shortcode option:selected").val();requested=badgeos_set_current_attributes(shortcode);window.send_to_editor(badgeos_construct_shortcode(requested))}function badgeos_construct_shortcode(requested){var shortcode="[";shortcode+=requested.shortcode;if(requested.params){for(i=0;i<requested.params.length;i++)shortcode+=" "+requested.params[i];$.trim(shortcode)}shortcode+="]";return shortcode}function s2formatResult_users(item){return item.user_login}
function s2formatSelection_users(item){return item.user_login}function s2formatResult_posts(item){return item.post_title}function s2formatSelection_posts(item){return item.post_title}$("#select_shortcode").on("change",function(){var selected=$("#select_shortcode option:selected").val();$(".badgeos_input").parent().hide();$("."+selected).parent().parent().show()});$("#badgeos_insert").on("click",function(e){badgeos_insert_shortcode()});$("#badgeos_cancel").on("click",function(e){e.preventDefault();
tb_remove()});$("#insert_badgeos_shortcodes").on("click",function(e){var inputs=$(".select2-container");$.each(inputs,function(index,el){$(el).select2("val","")})});$("#badgeos_achievements_list_user_id").select2({ajax:{url:ajaxurl,type:"POST",data:function(term){return{q:term,action:"get-users"}},results:function(data,page){return{results:data.data}}},id:function(object){return object.ID},allowClear:true,placeholder:badgeos_shortcode_embed_messages.id_placeholder,formatResult:s2formatResult_users,
formatSelection:s2formatSelection_users});$("#badgeos_achievement_id,#badgeos_nomination_achievement_id,#badgeos_submission_achievement_id").select2({ajax:{url:ajaxurl,type:"POST",data:function(term){return{q:term,action:"get-posts"}},results:function(data,page){return{results:data.data}}},id:function(object){return object.ID},allowClear:true,placeholder:badgeos_shortcode_embed_messages.id_placeholder,formatResult:s2formatResult_posts,formatSelection:s2formatSelection_posts});$("#badgeos_achievements_list_include,#badgeos_achievements_list_exclude").select2({ajax:{url:ajaxurl,
type:"POST",data:function(term){return{q:term,action:"get-posts",post_type:$("#badgeos_achievements_list_type").val()}},results:function(data,page){return{results:data.data}}},id:function(object){return object.ID},allowClear:true,placeholder:badgeos_shortcode_embed_messages.id_placeholder,multiple:true,formatResult:s2formatResult_posts,formatSelection:s2formatSelection_posts})})(jQuery);
