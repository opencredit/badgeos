 ...
 */ 
registerBlockType( 'bos/badgeos-submission-list-block', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __( 'Submission List - block' ), // Block title.
	 ...
	 */
	edit: ( props ) => { 
		const { attributes: { limit, status, show_filter, show_search, show_attachments, show_comments }, setAttributes} = props;
		...
		return [
			el("div", {
			  ...
			  el( ServerSideRender, {
				block: 'bos/badgeos-submission-list-block',
				attributes: props.attributes
			  } )
		  ),
		  <InspectorControls>
                <PanelBody title={ __( 'Sumission list', 'badgeos' ) } className="bos-block-inspector">
					<PanelRow> 
						<TextControl
					...
					<PanelRow> 
						<SelectControl
								label={ __( 'Show attachments', 'badgeos' ) }
								value={ show_attachments }
								options={ [
									{ label: __( 'True', 'badgeos' ), value: 'true' },
									{ label: __( 'False', 'badgeos' ), value: 'false' }
								] }
								onChange={ ( show_attachments ) => { setAttributes( { show_attachments } ) } }
							/>
                    </PanelRow>
					<PanelRow> 
						<SelectControl
								label={ __( 'Show comments', 'badgeos' ) }
								value={ show_comments }
								options={ [
									{ label: __( 'True', 'badgeos' ), value: 'true' },
									{ label: __( 'False', 'badgeos' ), value: 'false' }
								] }
								onChange={ ( show_comments ) => { setAttributes( { show_comments } ) } }
							/>
                    </PanelRow>
                </PanelBody>
			</InspectorControls>
