 ...
 */
registerBlockType('bos/badgeos-user-earned-ranks-block', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __('User Earned Ranks - block'), // Block title.
	...
	keywords: [
		__('User Earned Ranks - block'),
		__('block'),
		__('User Earned Ranks'),
	],
	supports: {
	 ...
	 */
	edit: (props) => {
		const { attributes: { rank_types, limit, show_search, order, orderby, user_id, show_description, show_thumb, show_title, default_view, image_width, image_height }, setAttributes } = props;
		let ranks = [];
		...
		let selectedRanks = [];
		if (null !== rank_types && rank_types != '') {
			selectedRanks = JSON.parse(rank_types);
		}

		//const handleRoleChange = ( role ) => setAttributes( { role: JSON.stringify( role ) } );
		function handleRankChange(rank_types) {
			props.setAttributes({ 'rank_types': JSON.stringify(rank_types) })
		}
